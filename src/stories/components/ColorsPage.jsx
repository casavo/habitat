  ...
  return <>{titles
    ...
    <section key={index} style={{ margin: `${space[300]} 0` }}>
      ...
      >
        ...
        <tbody>{getColors(values,title).map((e, ii, list) => <tr key={ii}>
          ...
          <td style={isLast(ii, list) ? padding: cell}>
            <Body noMargin={true}><code>vars.colors.{values}.{title}[{list[ii]}]</code></Body>
          </td>
          <td style={isLast(ii, list) ? padding: cell}>
            <Body noMargin={true}>
            <span style={{
              ...
              width: space[200],
            }}></span>{getHex(values, title, list[ii])}
          </Body>
          </td>
        </tr>)}</tbody>
      </table>
